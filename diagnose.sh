...
echo ""
echo "2. 检查端口..."
if netstat -tlnp 2>/dev/null | grep -q ":4090" || ss -tlnp 2>/dev/null | grep -q ":4090"; then
    echo "   ✓ 端口 4090 正在监听"
else
    echo "   ✗ 端口 4090 未监听"
fi
...
echo ""
echo "3. 测试登录 API..."
RESPONSE=$(curl -s -X POST http://localhost:4090/api/login \
...
echo "   curl -X POST http://localhost:4090/api/login \\"
echo "     -H 'Content-Type: application/json' \\"
echo "     -d '{\"username\":\"admin\",\"password\":\"admin\"}'"
echo ""
echo "登录信息:"
echo "  URL: http://localhost:4090"
echo "  用户名: admin"
echo "  密码: admin"
echo ""
